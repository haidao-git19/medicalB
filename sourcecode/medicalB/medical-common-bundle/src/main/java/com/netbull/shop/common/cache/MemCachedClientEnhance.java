...
public class MemCachedClientEnhance {
	private static final JLogger logger = LoggerFactory.getLogger("biz.wss.cache");
	...
	public boolean set(String key, Object value, Date expiry) {
		logger.debug("set memcached. key=" + key + ", value=" + value.getClass() + ", expiry=" + expiry);
		...
		try{
			...
			//If dataLen less then specified block size, will cached the value directly.
			logger.debug("set memcached. key=" + key + ", data length=" + dataLen + ", DefBlockSize=" + BLOCK_SIZE);
			...
			if(dataLen > BLOCK_SIZE) {
				...
				for(int i=1; i<=blockCount; i++) {
					...
					memcachedClient.set(blockKey, getBlockData(data, begin, end), expiry, blockKey.hashCode());
				}
			...
			try { if(bos != null) bos.close(); } catch (Exception e1) {/* nothing can do. */}
			try { if(oos != null) oos.close(); } catch (Exception e2) {/* nothing can do. */}
		}
	}
	...
	public Object get(String key) {
		...
		if(blockCount <= 0) {
			...
			Object obj = memcachedClient.get(key, key.hashCode());
			logger.warn("从缓存中获取信息对象：" + obj);
			return obj;
		}
		...
		try{
			...
			logger.debug("readObject from memory, obj info=" + (obj == null?"null":obj.getClass().getName()));
			return obj;
		}catch(Exception e) {
			...
			try { if(bos != null) bos.close(); } catch (Exception e1) {/* nothing can do. */}
			try { if(ois != null) ois.close(); } catch (Exception e2) {/* nothing can do. */}
		}
	}
}