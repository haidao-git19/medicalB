...
public class PharmacyController {
	...
	public Map<String, Object> query(Integer sEcho, Integer iColumns,
		...
		if (company == null) {
			requestMap.put("companyId", "0");
		} else {
			requestMap.put("companyId", company.getCompanyId() + "");
		}
		Page page = pharmacyService.page(iDisplayStart, iDisplayLength,
				requestMap);
	...
	@ResponseBody
	public String addOrUpdate(Integer shopID, Pharmacy pharmacy,
	...
	public String initAddOrUpdate(ModelMap model, Integer shopID,
			HttpServletRequest request) {
		// 查询当前账号的医药公司ID
		ShiroUser currUser = BaseDao.queryCurrentShiroUser();
		Map parameter1 = new HashMap();
		...
		ph.setCompanyId(company.getCompanyId());
		model.addAttribute("ph", ph);
		return "pharmacy/_initAddOrUpdate";
	}

	// @RequestMapping(value="/patient/showPharmacy")
	// public String showPharmacy(Pharmacy pharmacy,HttpServletRequest
	...
	@ResponseBody
	public String del(Integer shopID, HttpServletRequest request,
			HttpServletResponse response) {
		JSONObject json = new JSONObject();
		int count = 0;
	...
	public String createTemplate(ModelMap model, Integer id,
			HttpServletRequest request) {
		if (id != null && id > 0) {
			Map param = new HashMap();
			param.put("shopID", id);
	...
	@ResponseBody
	public Map<String, String> createDetail(Integer id,
			HttpServletRequest request) throws IOException {
		Map<String, String> requestMap = RequestUtils.parameterToMap(request);
		Map<String, String> resultMap = new HashMap<String, String>();

		String detail = requestMap.get("detail");
		String realPath = ConfigLoadUtil.loadConfig().getPropertie(
				"templateUploadPath");
		String fileName = StringTools.getBillno() + ".html";
		FileUtils.copyInputStreamToFile(
				new ByteArrayInputStream(detail.getBytes("UTF-8")), new File(
						realPath, fileName));
		String url = ConfigLoadUtil.loadConfig().getPropertie("templateUrl")
				+ fileName;
