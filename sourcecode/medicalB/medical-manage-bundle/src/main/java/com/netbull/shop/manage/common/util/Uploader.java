...
public class Uploader {
	...
	public void upload() throws Exception {
		...
		if(StringUtil.isEmpty(session_type)){
			if(StringTools.equals(session_type, "goods")){
				Integer compressWidth=Integer.parseInt(ConfigLoadUtil.loadConfig().getPropertie("imageWidth"));
				Integer compressHeight=Integer.parseInt(ConfigLoadUtil.loadConfig().getPropertie("imageHeight"));
				String realPath=ConfigLoadUtil.loadConfig().getPropertie("accessUrl");
				String uploadPath=ConfigLoadUtil.loadConfig().getPropertie("materialUploadPath");
				ImageCompressUtil.getInstance().compressPic(uploadPath, uploadPath, originalName, "S_"+originalName, compressWidth, compressHeight, false);
				savePicFileAttr(realPath+originalName, size, title, realPath+"S_"+originalName, state, "");
				session.invalidate();
			}
		}
	}
	 ...
	 * @param fileContent
	 */
	public void savePicFileAttr(String bigImgPath, int fileSize, String fileDesc,
			String smallImgPath, String state, String fileContent) {
		HttpSession session = request.getSession(true);
		String goodsCode = (String) session.getAttribute("goodsCode");
		Integer sort=Integer.parseInt(StringUtil.getString(session.getAttribute("sort")));
		...
		try {
			...
			sta = conn
					.prepareStatement("insert into tb_product_goods_image (goodsCode,bigImgPath,smallImgPath,sort) values (?,?,?,?)");
			sta.setString(1, goodsCode);
			sta.setString(2, bigImgPath);
			sta.setString(3, smallImgPath);
			sta.setInt(4, sort);
			sta.executeUpdate();
