...
public class OrderController {
	...
	public String putServiceOrder(HttpServletRequest reqest) throws Exception {
		...
		PutServiceOrderReq req = JSON.parseObject(reqJson, PutServiceOrderReq.class);
	...
	@ResponseBody
	public String updateOrderPayment(HttpServletRequest reqest) throws Exception {
		String reqJson = RequestUtil.getPostData(reqest);
		logger.info("req=" + reqJson);
		
		UpdateOrderPaymentReq req = JSON.parseObject(reqJson, UpdateOrderPaymentReq.class);
		Resp resp = orderService.updateOrderPayment(req);
		String rslt = JSON.toJSONString(resp);
		if(Constants.SUCCESS.equals(resp.getCode())) {
			ServiceOrder serviceOrder = orderService.findServiceOrder(req.getOrderNum());
			rslt = businessPayService.modifyBizStatus(String.valueOf(serviceOrder.getServiceType()), String.valueOf(serviceOrder.getServiceNumber()), SUC_BIZ_STATE);	
		}
		return rslt;
	}
	...
	public String patientOrderList(HttpServletRequest reqest) throws Exception {
		...
		PatientOrderListReq req = JSON.parseObject(reqJson, PatientOrderListReq.class);
	...
	public String doctorOrderList(HttpServletRequest reqest) throws Exception {
		...
		DoctorOrderListReq req = JSON.parseObject(reqJson, DoctorOrderListReq.class);
	...
	public String shopOrderList(HttpServletRequest reqest) throws Exception {
		...
		ShopOrderListReq req = JSON.parseObject(reqJson, ShopOrderListReq.class);
